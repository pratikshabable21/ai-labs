...
public class Astareightpuzzle
{
    ...
    static List<String> solution(int[][] initial, int[][] goal)
    {
     Set<int[][]> visited=new HashSet<>();
      ...
      visited.add(initial);
      queue.add(new state(initial, calculateHeuristic(initial, goal), 0));
      ...
      while(!queue.isEmpty())
      {
        ...
        for(int[] move: moves)
        {
           ...
           {
            queue.add(new state(next, calculateHeuristic(next, goal), curr.depth + 1));
            visited.add(next);
    ...
    public static class state implements Comparable<state> 
    {
        ...
        state(int[][] grid, int heuristic, int depth)
        {
            this.matrix = grid;
        ...
        {
            return Integer.compare(this.heuristic+this.depth, other.heuristic+this.depth);
        }
    }
